...
def generate_recommendation_list(
    ...
    if recommendation_path is None or len(recommendation_path) == 0:
        ...
        recommendation_file = RecommendationMem(
            eval_strategy.get_report_db(), n_reports
        )
        # print(n_reports)
    elif sparse:
        if reuse > 0:
            logging.getLogger().info("Reusing file sparse")
            old_recommendation_gen = RecommendationFileSparse(
                eval_strategy.get_report_db(),
                recommendation_path,
                n_reports,
                only_read=True,
            ).read_file()
        ...
        recommendation_file = RecommendationFileSparse(
            eval_strategy.get_report_db(), recommendation_path, n_reports
        )
    else:
        logging.getLogger().info("RecommendationFileH5")
        recommendation_file = RecommendationFileH5(
            recommendation_path, n_reports, max_length=max_length
        )

    scorer.preprocess_reports(
        eval_strategy.get_report_set(), eval_strategy.get_queries()
    )
            ...
            if len(candidates) == 0:
                if isinstance(eval_strategy, SunStrategy):
                    ...
                    logging.getLogger().warning(
                        "Bug {} has 0 candidates!".format(query_id)
                    )
                else:
    ...
    logger.info(
        "Time to generate recommendation of {} reports: {}".format(
            n_reports, (time() - start_time)
        )
    )
    recommendation_file.flush()
...
class RecommendationFixer(object):
    ...
    def fix(self, report_id, candidates, scores):
        ...
        if len(candidates) > len(correct_candidates):
            raise Exception(
                "recommendation list of {} contains invalid candidates".format(
                    report_id
                )
            )
        ...
        if len(candidates) < len(correct_candidates):
            ...
            if not self.add_cand:
                raise Exception(
                    "Candidates {} are out of recommendation list".format(
                        missing_candidates
                    )
                )
...
class RecommendationFile(object):
    ...
    def iterate(self, strategy, listeners, fixer):
        ...
        for report_id, candidates, scores in self:
            ...
            if len(candidates) == 0:
                logging.getLogger().warning(
                    "Bug {} has 0 candidates!".format(report_id)
                )
                continue
        ...
        if len(queries - tested_queries) > 0:
            for t in queries - tested_queries:
                if not strategy.is_to_ignore(t):
                    raise Exception(
                        "Missing queries: {}".format(queries - tested_queries)
                    )
...
class RecommendationFileH5(RecommendationFile):
    ...
    def __init__(self, filepath, n_reports, only_read=False, max_length=-1):
        ...
        if only_read:
            ...
            if os.path.exists(filepath):
                ...
                logger.warning(
                    "{} exists. The content will redirected to {}".format(
            ...
            self.dset = self.file.create_dataset(
                "recommendation_list", (n_reports, 3), dtype=dt
            )
            self.idx = 0
    ...
    def add(self, report_id, recommendation_list):
        ...
        r_iter = (
            islice(recommendation_list, self.max_length)
            if self.max_length > 0
            else iter(recommendation_list)
        )
...
class SunStrategy(object):
    ...
    def __init__(self, report_db, dataset, window):
        ...
        newest_report = (
            self.queries[0],
            read_date_from_report(self.report_db.get_report(self.queries[0])),
        )
        ...
        for report_id in self.queries:
            ...
            if newest_report[1] < creation_date or (
                newest_report[1] == creation_date and report_id > newest_report[0]
            ):
        ...
        for report in self.report_db.report_list:
            ...
            if creation_date > newest_report[1] or (
                creation_date == newest_report[1]
                and report["bug_id"] > newest_report[0]
            ):
                continue
        ...
        for masterId, masterSet in self.report_db.get_master_set_by_id(
            ...
            for report_id in masterSet:
                if masterId > report_id:
                    raise Exception(
                        "MasterId {} is greater than duplicate {}".format(
                            masterId, report_id
                        )
                    )

                bug_creation_date = read_date_from_report(
                    self.report_db.get_report(report_id)
                )
    ...
    def get_candidate_list(self, query_id):
        ...
        for master_id, ts_master_set in self.creation_dates_by_master_id.items():
            ...
            for report_id, ts in ts_master_set:
                ...
                if ts > query_timestamp or (
                    ts == query_timestamp and report_id >= query_id
                ):
                    continue
